...
if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
    ...
    recognition.onresult = (event) => {
        ...
        // Only update if there's new content to avoid clearing current text
        if (finalTranscript) {
            transcriptArea.innerText += (transcriptArea.innerText ? ' ' : '') + finalTranscript;
        }
        
        // We could display interim text in a different style, but for simplicity:
        // statusText.innerText = interimTranscript || '正在聆聽...';
    };
    ...
    recognition.onend = () => {
        if (isRecording) {
            // Restart if it stopped unexpectedly but we're still in "recording" mode
            recognition.start();
        } else {
            stopRecording();
        }
...
function startRecording() {
    ...
    transcriptArea.innerText = ''; // Optional: Clear on start
    recognition.lang = currentLang;
    recognition.start();
}
